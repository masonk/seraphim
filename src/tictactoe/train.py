...
print(os.environ.get("SERAPHIM_MODEL_NAME"))
print(os.environ.get("SERAPHIM_DATA"))

parser = argparse.ArgumentParser(
    description="Initialize a TicTacToe expert model.")
...
def make_dataset(minibatch, dataset_dir):
    ...
    try:
        ...
        while len(files) == 0:
            i += 1
            if i % 12 == 0:
                print("No files; waiting for some to be generated")
            sleep(5)
            files = glob.glob("{}/*.tfrecord".format(dataset_dir))
...
def write_snapshot(sess, saver, global_step_val, snapshot_id=None):
    if snapshot_id is None:
        snapshot_id = "{}-{}".format(
            datetime.datetime.now().replace(microsecond=0).isoformat(), global_step_val
        )
    snapshot_dir = model_dir + "/" + snapshot_id
    saver_dir = snapshot_dir + "/checkpoints/"
...
def save_savedmodel(sess, snapshot_dir):
    ...
    # }
    # serving_inputs = {"example": build_tensor_info(example)}
    # signature_outputs = {"softmax": build_tensor_info(softmax)}

    # training_signature_def = build_signature_def(
    #     training_inputs, signature_outputs, REGRESS_METHOD_NAME
    # )
...
def train(sess):
    ...
    train_op = tf.get_collection("train")[0]
    global_step = tf.get_collection("global_step")[0]
    increment_global_step = tf.get_collection("increment_global_step")[0]
    merged_summaries = tf.get_collection("merged_summaries")[0]
    write_training_summaries = tf.summary.FileWriter(
        model_dir + "/champion/summaries", sess.graph
    )
    ...
    with catch_sigint() as got_sigint:
        while True:
            ...
            for i in range(snapshot_epochs):
                ...
                while True:
                    if got_sigint():
                        break
                    try:
                        global_step_val = sess.run(increment_global_step)
                        e = sess.run(example_it)
                        l = sess.run(label_it)
                        feed = {example_ph: e, label_ph: l, training_ph: False}
                        if global_step_val % 10 == 0:
                            summary = sess.run(
                                merged_summaries, feed_dict=feed)
                            write_summaries(
                                write_training_summaries, summary, global_step_val
                            )
            ...
            global_step_val = sess.run(global_step)
            print(
                "EPOCH {} FINISHED ({} minibatches of {} examples)".format(
                    epoch, global_step_val, minibatch_size
                ),
                flush=True,
            )
...
def init_model(sess):
    ...
    # we have to add an iterator handle to the graph so that training can feed the net by handle on
    ...
    training = tf.placeholder(
        tf.bool, name="training", shape=()
    )  # true during training, false during inference

    tf.add_to_collection("example", example)
    tf.add_to_collection("label", label)
    tf.add_to_collection("training", training)
    dense = tf.layers.dense(
        tf.cast(example, tf.float32), units=128, activation=tf.nn.relu
    )

    for i in range(dense_layers):
        batch_norm = tf.layers.batch_normalization(
            dense, training=training, renorm=True, trainable=True
        )
        dense = tf.layers.dense(batch_norm, units=92, activation=tf.nn.relu)

    logits = tf.layers.dense(
        dense, units=9, activation=tf.nn.relu, name="logits")
    ...
    tf.add_to_collection("increment_global_step", increment_global_step_op)
